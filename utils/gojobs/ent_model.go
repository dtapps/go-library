...
func EntTaskFields() []ent.Field {
	return []ent.Field{
		...
		field.String("spec").Comment("任务规则").Annotations(entsql.WithComments(true)),
		...
		field.Int64("frequency").Optional().Comment("频率(秒单位)").Annotations(entsql.WithComments(true)),
		...
		field.String("custom_id").Immutable().Optional().Comment("自定义编号").Annotations(entsql.WithComments(true)),
...
func EntTaskLogFields() []ent.Field {
	return []ent.Field{
		...
		field.Time("log_time").Immutable().Default(time.Now).Comment("日志时间").Annotations(entsql.WithComments(true)),
		field.Int64("task_id").Immutable().Comment("任务编号").Annotations(entsql.WithComments(true)),
		field.String("task_run_id").Immutable().Optional().Comment("执行编号").Annotations(entsql.WithComments(true)),
		field.Int("task_result_code").Optional().Comment("执行状态码").Annotations(entsql.WithComments(true)),
		field.String("task_result_desc").Optional().Comment("执行结果").Annotations(entsql.WithComments(true)),
		field.String("system_inside_ip").Optional().Default("0.0.0.0").Comment("内网IP").Annotations(entsql.WithComments(true)),
		field.String("system_outside_ip").Optional().Default("0.0.0.0").Comment("外网IP").Annotations(entsql.WithComments(true)),
