...
func BindURLQueryToStruct(params map[string]any, obj any) error {
	...
	if v.Kind() != reflect.Ptr || v.IsNil() {
		return fmt.Errorf("BindURLQueryToStruct: obj must be a non-nil pointer")
	}
	v = v.Elem()
	t := v.Type()
...
func parseFields(t reflect.Type, parentIndex []int) []fieldMeta {
	...
	for i := 0; i < t.NumField(); i++ {
		...
		fields = append(fields, fieldMeta{index: index, tag: tagName})
	}
	return fields
}
...
func setField(field reflect.Value, strVal string) error {
	...
	case reflect.Int, reflect.Int8, reflect.Int16, reflect.Int32, reflect.Int64:
		...
		if err != nil {
			return fmt.Errorf("failed to parse int value '%s': %v", strVal, err)
		}
		field.SetInt(num)
	case reflect.Uint, reflect.Uint8, reflect.Uint16, reflect.Uint32, reflect.Uint64:
		num, err := strconv.ParseUint(strVal, 10, 64)
		if err != nil {
			return fmt.Errorf("failed to parse uint value '%s': %v", strVal, err)
		}
		field.SetUint(num)
	case reflect.Bool:
		b, err := strconv.ParseBool(strVal)
		if err != nil {
			return fmt.Errorf("failed to parse bool value '%s': %v", strVal, err)
		}
		field.SetBool(b)
	case reflect.Float32, reflect.Float64:
		num, err := strconv.ParseFloat(strVal, field.Type().Bits())
		if err != nil {
			return fmt.Errorf("failed to parse float value '%s': %v", strVal, err)
		}
		field.SetFloat(num)
	default:
