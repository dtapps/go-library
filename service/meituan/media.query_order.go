...
type MediaQueryOrderResponse struct {
	...
	Data    []struct {
		...
		dataList struct {
			businessLine      int64  `json:"businessLine"`      // 业务线，同入参枚举说明
			...
			} `json:"orderDetail"` // 订单详情，只支持到家外卖商品券、到家医药、到家闪购商品业务、到店到餐、到综、酒店类型返回数据
			...
			tradeType       string `json:"tradeType"`       // 交易类型，1：cps，2：cpa
			actId           int64  `json:"actId"`           // 活动物料id，我要推广-活动推广中第一列的id信息
			appkey          string `json:"appkey"`          // 归因到的appKey，对应取链时入参的appkey
			skuCount        int64  `json:"skuCount"`        // 表示sku数量，团好货和券包类型的CPS订单返回有值，其余类型订单不返回该值
