...
func (c *Client) FundAppMchTransferTransferBills(ctx context.Context, notMustParams ...*gorequest.Params) (response FundAppMchTransferTransferBillsResponse, apiError ApiError, err error) {
	...
	params.Set("appid", c.GetAppId()) // 【商户AppID】 是微信开放平台和微信公众平台为开发者的应用程序(APP、小程序、公众号、企业号corpid即为此AppID)提供的一个唯一标识
	...
	// 请求
	err = c.DoRequest(ctx, "/v3/fund-app/mch-transfer/transfer-bills", params, http.MethodPost, &response, &apiError)
	return
}
