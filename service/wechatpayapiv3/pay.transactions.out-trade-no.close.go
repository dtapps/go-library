...
func (c *Client) PayTransactionsOutTradeNoClose(OutTradeNo string) *PayTransactionsOutTradeNoCloseResult {
	// 参数
	params := gorequest.NewParams()
	params["mchid"] = c.GetMchId()
	// 	请求
	request, err := c.request(fmt.Sprintf(apiUrl+"/v3/pay/transactions/out-trade-no/%s/close", OutTradeNo), params, http.MethodPost, false)
