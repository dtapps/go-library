...
func newWeather7DResult(result Weather7DResponse, body []byte, http gorequest.Response) *Weather7DResult {
	return &Weather7DResult{Result: result, Body: body, Http: http}
}
...
func (c *Client) Weather7D(ctx context.Context, location string, notMustParams ...gorequest.Params) (*Weather7DResult, error) {
	...
	// 请求
	request, err := c.request(ctx, "weather/7d", params, http.MethodGet)
	if err != nil {
		return newWeather7DResult(Weather7DResponse{}, request.ResponseBody, request), err
	}
	// 定义
	var response Weather7DResponse
	err = gojson.Unmarshal(request.ResponseBody, &response)
	return newWeather7DResult(response, request.ResponseBody, request), err
}
