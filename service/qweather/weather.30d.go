...
func newWeather30DResult(result Weather30DResponse, body []byte, http gorequest.Response) *Weather30DResult {
	return &Weather30DResult{Result: result, Body: body, Http: http}
}
...
func (c *Client) Weather30D(ctx context.Context, location string, notMustParams ...gorequest.Params) (*Weather30DResult, error) {
	...
	// 请求
	request, err := c.request(ctx, "weather/30d", params, http.MethodGet)
	if err != nil {
		return newWeather30DResult(Weather30DResponse{}, request.ResponseBody, request), err
	}
	// 定义
	var response Weather30DResponse
	err = gojson.Unmarshal(request.ResponseBody, &response)
	return newWeather30DResult(response, request.ResponseBody, request), err
}
