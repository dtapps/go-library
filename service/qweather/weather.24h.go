...
func newWeather24HResult(result Weather24HResponse, body []byte, http gorequest.Response) *Weather24HResult {
	return &Weather24HResult{Result: result, Body: body, Http: http}
}
...
func (c *Client) Weather24H(ctx context.Context, location string, notMustParams ...gorequest.Params) (*Weather24HResult, error) {
	...
	// 请求
	request, err := c.request(ctx, "weather/24h", params, http.MethodGet)
	if err != nil {
		return newWeather24HResult(Weather24HResponse{}, request.ResponseBody, request), err
	}
	// 定义
