...
func newWeather15DResult(result Weather15DResponse, body []byte, http gorequest.Response) *Weather15DResult {
	return &Weather15DResult{Result: result, Body: body, Http: http}
}
...
func (c *Client) Weather15D(ctx context.Context, location string, notMustParams ...gorequest.Params) (*Weather15DResult, error) {
	...
	// 请求
	request, err := c.request(ctx, "weather/15d", params, http.MethodGet)
	if err != nil {
		return newWeather15DResult(Weather15DResponse{}, request.ResponseBody, request), err
	}
	// 定义
	var response Weather15DResponse
	err = gojson.Unmarshal(request.ResponseBody, &response)
	return newWeather15DResult(response, request.ResponseBody, request), err
}
