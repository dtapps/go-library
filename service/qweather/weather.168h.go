...
func newWeather168HResult(result Weather168HResponse, body []byte, http gorequest.Response) *Weather168HResult {
	return &Weather168HResult{Result: result, Body: body, Http: http}
}
...
func (c *Client) Weather168H(ctx context.Context, location string, notMustParams ...gorequest.Params) (*Weather168HResult, error) {
	...
	// 请求
	request, err := c.request(ctx, "weather/168h", params, http.MethodGet)
	if err != nil {
		return newWeather168HResult(Weather168HResponse{}, request.ResponseBody, request), err
	}
	// 定义
	var response Weather168HResponse
	err = gojson.Unmarshal(request.ResponseBody, &response)
	return newWeather168HResult(response, request.ResponseBody, request), err
}
