...
func (c *Context) BindJsonAndValidate(obj any) error {
	if c.IsGin() {
		...
		if isDebug {
			fmt.Printf("Gin 绑定 Body JSON 参数:%+v\n", obj)
		}
		...
		if isDebug {
			fmt.Printf("Gin 设置默认值:%+v\n", obj)
		}
	}
	if c.IsHertz() {
		if isDebug {
			fmt.Printf("Hertz 绑定:%+v\n", obj)
		}
		if err := c.hertzCtx.Bind(obj); err != nil {
			return fmt.Errorf("参数绑定失败：%w", err)
		}
		if isDebug {
			fmt.Printf("Hertz 绑定 参数:%+v\n", obj)
		}
	}
